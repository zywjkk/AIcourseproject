...
def _append_summary(config: ExperimentConfig, metrics: dict[str, float], path: Path) -> None:
    ...
    # Read existing rows, drop any stale entry for this experiment
    rows: list[dict[str, str]] = []
    if path.exists():
        with path.open("r", encoding="utf-8", newline="") as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row.get("experiment") != config.experiment_name:
                    rows.append(row)
        # Use fieldnames from the existing file if compatible, else our own
